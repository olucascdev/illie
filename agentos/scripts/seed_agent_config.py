...
Inserts the default Illie agent instructions into the agent_config table.
Run once after creating the table in NeonDB.
...
def seed():
    ...
    with conn.cursor() as cur:
        cur.execute(
            """
            INSERT INTO agent_config (id, prompt)
            VALUES (1, %s)
            ON CONFLICT (id)
    ...
    conn.commit()
    conn.close()
    print("✅ agent_config seeded successfully.")
